...
Recorded as **[D-19](decisions.md#d-19)**. The two-tier choice:
...
- Two tiers separates the concerns: fast feedback for development, rigorous coverage for release. Same scoring engine, same scenario-manifest schema, same StrideGPT-comparison protocol ([D-16](decisions.md#d-16)) only the catalogues differ.
...
| **comparison vs. StrideGPT** | same metrics computed against StrideGPT's output for the same scenarios (per [D-16](decisions.md#d-16)) |
...
Per [D-16](decisions.md#d-16), StrideGPT is our quality benchmark. For each scenario we run *both* tools and tabulate:
