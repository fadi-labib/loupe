...
Implementation timing: Tier 1 landed — `benchmarks/tier1-mongoose/` (9 independently-verified historical-CVE scenarios) and the shared `benchmarks/scoring/` engine are in the repo, runnable via `uv run python benchmarks/run-tier1.py`. Tier 2 (`benchmarks/tier2-mosquitto/`) remains deferred to Phase 10; `benchmarks/scoring/` was built tier-agnostic (nothing in it references "mongoose" or "tier1") so Tier 2 can plug in without re-architecture when it lands. Contributors can write Tier 2 scenarios today against the documented schema. The shared scoring methodology is publishable as an industry rubric; other STRIDE-tool authors can run the same scenarios against their own tools.

A cadence clarification, since "runs per Loupe PR" could otherwise read as a required PR-blocking status check, which Tier 1 deliberately is not: every other required CI check in this repo (ruff, mypy, pytest-with-empty-keys, lychee, vale) is free and deterministic, and Tier 1 is neither — it's a live-LLM run with real API cost, scored against curated expectations rather than a byte-for-byte cassette replay. "Per PR" means a contributor runs `uv run python benchmarks/run-tier1.py` on demand before pushing a change to the agent loop, prompt, or run-record shape, as a recommended habit; a separate scheduled `benchmark-tier1` GitHub Actions workflow (weekly + `workflow_dispatch`) provides maintainer-controlled-cadence tracking over time. Nothing about this gates a PR's merge automatically. See [`benchmarks/README.md`](https://github.com/fadi-labib/loupe/blob/main/benchmarks/README.md) for the full reasoning.
...
## D-23: split `requires_capabilities` from `prefers_capabilities` and enforce both { #d-23 }

The candidates, surfaced after the Mongoose end-to-end validation (gap log §F-10):

- **Option A (chosen):** split the lens contract into two fields. `requires_capabilities` means the lens does not run without the capability — bootstrap raises, the dispatcher records a `lens_skipped` finding, and the run is treated as evidence-incomplete. `prefers_capabilities` means the lens runs degraded — the slot stays `None`, the run record carries a structured `capability_degraded` entry naming the capability, kind (unconfigured / backend_error / not_found / entry_point_malformed), and detail.
- **Option B (rejected):** hard-fail on every capability error, no degraded mode. Aligns with the strictest reading of the field name and removes the silent-warning code path entirely. Rejected because the first `loupe ci` after `loupe init` (which ships `capabilities:` commented out by design — commit `1ef622c`) would hard-fail before the user has had a chance to learn what wiring up means.
- **Option C (rejected):** keep the warning, mark the run as `degraded: true` in the run record, add `loupe verify --no-degraded`. Rejected because the contract is still violated by default; anyone who doesn't pass `--no-degraded` keeps getting false-green results. "Looks like a fix" without being one.
- **Option D (rejected):** promote `requires_capabilities` failures to per-lens skips but keep `loupe ci` exit 0. Rejected because silent green CI for a lens that didn't run is the same false-confidence failure mode the strict `check_lens_liveness` policy in `gating.py` already exists to prevent.

What this fixes: before this decision, every `CapabilityError` subclass — `NoBackendsConfiguredError`, `BackendError`, `CapabilityNotFoundError`, `EntryPointMalformedError` — was caught at the `ci_cmd` / `scan_cmd` boundary and printed as a warning. The lens then ran with `ctx.sbom = None` and `ctx.cve_findings = None` despite the contract declaring both as required. Threats produced under that path carried CWE references that came from training-data recall rather than a real Grype scan, and there was no auditor-visible signal that the lens had been degraded.

Implementation surface: new `prefers_capabilities` field on `LensCapabilities` (default empty list); new typed `RequiredCapabilityUnavailable` exception and `CapabilityDegradation` dataclass in `capabilities/errors.py`; bootstrap iterates the two lists separately; `ci_cmd` and `scan_cmd` drop the broad `except CapabilityError`; new `capability_degraded:` field on `RunRecord` for audit-grade preserved degradation history; `check_lens_liveness` extended to recognise `lens_skipped` findings. Plan: fix plan, Phase A.

Per-lens migration: ThreatLens keeps `requires_capabilities=["sbom", "cve"]` under the new contract — no migration to `prefers`. The bootstrap raises `RequiredCapabilityUnavailable` on an unwired capability; `ci_cmd` and `scan_cmd` translate this to exit 64 (BSD `EX_USAGE` — operator-configuration error), not exit 1. The `lens_skipped` finding still lands on the run record for audit-trail completeness, but exit code 1 stays reserved for runs where lenses produced findings that tripped `ci.fail_on`. This makes onboarding stricter: a fresh `loupe init` no longer produces a green `loupe ci` until the operator wires Syft + Grype. To keep the wall visible *before* the user hits it in `ci`, two additional changes ride in the same fix: `loupe init`'s post-scaffold message points at the capability-wiring step, and `loupe doctor`'s capability check escalates from `warn` to `fail` when any required capability is unwired. Full sequencing in the fix plan, Phase A.

The choice between `requires` and `prefers` was made deliberately in favour of audit-trail integrity over onboarding ergonomics. The first run after `loupe init` walking the user through capability wiring is a one-time cost; the alternative — first-run threats whose CWE references come from training-data recall, with no auditor-visible signal — has no expiration date. Once a real CRA-target downstream relies on these artefacts as evidence, the structured `capability_degraded` entry from a `prefers` path would still need to be promoted to a hard gate at some review checkpoint; making `requires` the default from day one collapses that review into the first-run UX where it can be addressed once.

The reason this is its own decision (not folded into D-11 / D-18): D-11 fixed the *shape* of the lens contract (six methods, one declared attribute); D-18 fixed the *abstraction* (capability protocols, registry, composition modes). What was missing was an explicit policy for what happens when the abstraction breaks down at the operator layer. That gap was unobservable until F-10 surfaced it during real end-to-end validation; recording the resolution here means a future lens author can see the contract was deliberate, not accidental.

---

## D-24: scoped `loupe scan` reads source files into a dedicated prompt section { #d-24 }

The candidates, surfaced after the Mongoose end-to-end validation (gap log §F-09):

- **Option A (chosen):** `scan_cmd` reads each `--paths` entry (directories expand recursively, filtered by a shared `CODE_EXTENSIONS` allow-list, per-file size cap), attaches the contents to a new `ctx.scoped_sources: list[ScopedSource]` field on `RunContext`, and the prompt builder adds a `_sources_section` rendered between `_diff_section` and `_sbom_section` — inside the stable prefix so Anthropic prompt-cache breakpoints still work (D-10).
- **Option B (rejected):** reuse `ctx.diff.raw_unified` by faking a unified diff that "adds" each file. Smallest implementation diff (no new fields, no new prompt section), but dishonest semantics that bleed into the audit trail: `diff_hash` in the run record becomes a hash of a fictional patch, the GitHub Action's sticky-comment "changes detected" framing fires for a scan, and Layer-3 verify (D-12) ends up validating a hash chain over fabricated diff bytes. Rejected on audit-trail grounds.
- **Option C (deferred to v1.x):** register a `read_source(path)` agent tool. The LLM decides which files to read and pulls them in as it reasons. Token-efficient at scale — right answer for Tier 2 (Mosquitto, ~70k LOC) where Option A would blow any reasonable budget. Deferred because Tier 2 isn't on the immediate roadmap, and Option A is sufficient for Tier 1 (Mongoose) where individual scoped files fit inside the per-run budget envelope.
- **Option D (rejected):** orchestrator-level fan-out — one agent run per file, dispatcher aggregates threats across runs. Heavy refactor of D-07 (blackboard), D-10 (cost levers — loses stable-prefix caching between chunks), and D-12 (hash-chain semantics for one scan → N sub-runs). Premature; revisit only if A + C combined cannot cover the Tier 2 budget envelope.

What this fixes: before this decision, `scan_cmd.scan_command` stored `scope_paths` on `RunContext` but no consumer ever read it. `user_prompt.py` reads `ctx.diff` / `ctx.project` / `ctx.sbom` / `ctx.cve_findings` / `ctx.knowledge` — none of which were populated from `scope_paths`. The agent received exactly: system prompt, `context.md`, the literal placeholder `"No diff provided. Operating in full-repo or scoped mode; reason about the project as a whole."`, the "no elements" placeholder, and the default focus instruction. No source code. The threats it produced were reasoned from `context.md` + training-data recall, not from the bytes of the files the operator scoped.

Implementation surface: new `ScopedSource(path, content, truncated_at)` dataclass and `scoped_sources` field on `RunContext`; new `safe_read_under(project_root, target, max_bytes)` helper in `enforcement/path_boundary.py` mirroring the F-02 write-side parent-chain walk (NOFOLLOW at every level so symlinks out of the project root are refused); `CODE_EXTENSIONS` moved from `loupe-threatlens` to `loupe_core.fs` for shared access; `scan_cmd` walks paths with directory recursion + extension filter + size cap (default 50,000 chars per file, configurable via `--max-chars-per-file`); `user_prompt._sources_section` renders the section, empty when no sources so CI mode prompts are byte-identical. Plan: fix plan, Phase B.
