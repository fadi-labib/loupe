...
: (Shipped.) Branch-namespace enforcement at the CI runner. The Action authenticates with a fine-grained `LOUPE_PAT` scoped `contents: write` only on refs matching `loupe/proposal-*`; CODEOWNERS gates `.loupe/context.md`, `.loupe/decisions/**`, `.loupe/config.yaml`, and `.loupe/knowledge.yaml`. The opt-in mechanism is `auto_commit_loupe_dir: true` plus the `LOUPE_PAT` secret; the Action fail-fasts (exit 64) if `auto_commit_loupe_dir=true` is set without a PAT. There is no `GITHUB_TOKEN` fallback for the push path — that's a deliberate refusal to allow a "soft Layer 2" via the broader-scope default token. See [D-08](decisions.md#d-08) and [D-27](decisions.md#d-27).

**Layer 3**{: #term-layer-3 }
: (Shipped.) `loupe verify`. Four checks run on every invocation: (1) hash-chain integrity across `.loupe/runs/*.json`, (2) artefact schema consistency for every `threats.yaml` / `mitigations.yaml`, (3) threats-to-mitigations cross-references (every `mitigation_ids` resolves; every `threats_addressed` resolves), and (4) protected-path authorship when invoked with `--strict` (also reachable as `verify_repo(..., strict=True)` from the library). Reachable as a pre-commit hook and as a required CI check.

**Layer 4**{: #term-layer-4 }
: (Shipped.) Interactive UX gate. `loupe chat` walks each staged `.loupe/.proposed/<...>.patch` through a `[y/N/edit/skip]` prompt with default-N, applies approved patches via `git apply`, and `git mv`s the .patch from `.proposed/` to `.applied/` (accept), `.skipped/` (explicit reject), or leaves it in place (defer). No `--auto-confirm` flag. The one exception to default-N is the visibility-only carry-over prompt at session start ("include unreviewed proposals from previous runs?"). See [D-26](decisions.md#d-26) for the related `.loupe/`-only git-write exception that makes `git apply` and `git mv` permissible from chat.
