...
**Full-repo mode** — `loupe scan`. The agent analyses the whole codebase as if it were a giant diff against an empty baseline. Used for onboarding, periodic re-baseline, audit prep. Higher cost than diff mode. See [`DESIGN-DECISIONS.md` D-15](DESIGN-DECISIONS.md#d-15--full-repo--no-diff-scans-are-first-class-via-loupe-scan).
