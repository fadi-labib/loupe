...
- Multi-LLM is a hard requirement (see [PRINCIPLES.md §7](PRINCIPLES.md#7-multi-llm-by-default-never-single-vendor-lock-in)). PydanticAI supports Anthropic, OpenAI, Google, Mistral, Groq, Cohere, Ollama, Bedrock natively.
...
- The "loupe" metaphor connotes precision inspection — auditor-credible work — which matches the values [§1 evidence not theatre](PRINCIPLES.md#1-produce-evidence-not-theatre).
...
- Naming reflects activity, not regulation (see [PRINCIPLES.md §10](PRINCIPLES.md#10-naming-reflects-activity-not-regulation)).
...
- Standards-conformance (CycloneDX, OpenVEX, STRIDE) is non-negotiable — see [PRINCIPLES.md §6](PRINCIPLES.md#6-standard-formats-over-proprietary-ones).
...
**Why:**
- The user explicitly requested "enforce by tooling, not policy" — see [PRINCIPLES.md §2](PRINCIPLES.md#2-the-write-boundary-is-a-python-function-not-a-prompt-instruction).
...
**Why:**
- Cost discipline was elevated from "nice to have" to "first-class design property" after the user's clarification. See [PRINCIPLES.md §4](PRINCIPLES.md#4-cost-discipline-is-a-first-class-design-property).
...
- See [PRINCIPLES.md §9](PRINCIPLES.md#9-test-discipline-vcr-or-no-llm-never-live-calls-in-ci).
...
**Why:**
- **Parity with the LLM-provider value.** [PRINCIPLES.md §7](PRINCIPLES.md#7-multi-llm-by-default-never-single-vendor-lock-in) commits us to no LLM-vendor lock-in via PydanticAI. The original v1 spec accidentally re-introduced lock-in at the tool layer (Syft hardcoded in `sbom.py`). Capabilities extend the same pattern down a layer.
