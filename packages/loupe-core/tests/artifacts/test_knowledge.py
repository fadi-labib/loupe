...
def test_knowledge_graph_with_entities(tmp_path):
    kg = KnowledgeGraph(
        ...
        assets=[Asset(
            id="A-001", name="payment_service",
            description="Processes payments", criticality="high",
            facts_supporting=["F-001"], first_identified=date(2026, 4, 2),
            confirmed_by_human=True,
        )],
        elements=[Element(id="E-001", name="api-gateway", type="trust_boundary",
                          interfaces=["ext:internet"])],
        decisions=[Decision(
            id="D-2026-04-15-skip-rate-limit", type="risk_acceptance",
